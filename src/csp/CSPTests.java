...
public class CSPTests {
    ...
    public static void testSolution(List<LocalDate> soln, Set<DateConstraint> constraints) {
        System.out.println("printing solution in test");
        for (int i = 0; i < soln.size(); i++) {
            System.out.println("index " + i);
            if (soln.get(i) == null) {
                System.out.println(" is null");
            } else {
                System.out.println(soln.get(i).toString());
            }
        }
        for (DateConstraint d : constraints) {
            ...
            switch (d.OP) {
                case "==":
                    if (leftDate.isEqual(rightDate)) sat = true;
                    break;
                case "!=":
                    if (!leftDate.isEqual(rightDate)) sat = true;
                    break;
                case ">":
                    if (leftDate.isAfter(rightDate)) sat = true;
                    break;
                case "<":
                    if (leftDate.isBefore(rightDate)) sat = true;
                    break;
                case ">=":
                    if (leftDate.isAfter(rightDate) || leftDate.isEqual(rightDate)) sat = true;
                    break;
                case "<=":
                    if (leftDate.isBefore(rightDate) || leftDate.isEqual(rightDate)) sat = true;
                    break;
            }
            if (!sat) {
